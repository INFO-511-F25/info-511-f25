...
-   Your code must use one of `melt()` or `pivot_table()`. There are other ways you can do this reshaping move in R, but this question requires solving this problem by pivoting.
...
Then, in a new pipeline, find the `unique()` countries in the data frame you created.
...
Use a short but informative name.
Then, in a new pipeline, find the `unique()` divisions in the data frame you created.
