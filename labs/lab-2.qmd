...
In addition to points (`sns.scatterplot()`), represent the data with a smooth curve fit to the data with `sns.lineplot()`, with the argument `ci=None` to remove the confidence bands.
Make sure to set informative titles, axis, and legend labels for your plot.
Which plot do you prefer - this plot or the plot in Question 3?
Briefly explain your choice.

::: callout-note
`ci=None` removes the confidence bands around the line.
...
#| echo: false
midwest = pd.read_csv('data/midwest.csv')
g = sns.FacetGrid(midwest, col="state", col_wrap=4, height=4, aspect=1)
g.map_dataframe(sns.scatterplot, x="percollege", y="percbelowpoverty", hue="percwhite", alpha=0.5, s=50)
g.add_legend()

g.set_axis_labels("% college educated", "% below poverty line")
g.set_titles("{col_name}")
g.fig.suptitle("ADD TITLE", y=1.02)
...
```{python}
#| eval: false
az_county = pd.read_csv('data/az_county.csv')
```
