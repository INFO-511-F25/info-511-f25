...
Of the models from Exercises 1 and 2, the model with the highest adjusted $R^2$ is the one with `nougat` removed.
Therefore, we will go to Exercise 4 eliminating one variable at a time from that model.
...
None of the models in Exercise 6 resulted in a higher adjusted $R^2_{adj}$.
Therefore our final model is the one selected in Exercise 5.
